...
def validate_and_grade(filepath):
    ...
    for row_idx, row in enumerate(ws_entry.iter_rows(min_row=2, values_only=True), start=2):
        ...
        student_id, name, dob = row[0], row[1], row[2]
        if not student_id: errors.append(f"Row {row_idx}: Missing Student ID")
        if not name: errors.append(f"Row {row_idx}: Missing Name")
